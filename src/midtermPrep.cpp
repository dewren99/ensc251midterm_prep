...
class TreeNode {
 ...
 protected:
  ...
  /**
   * // declares action_func as a type representing a pointer to a TreeNode
   member function that takes no arguments (void) and returns void.
   * class TreeNode {
          public:
          ...
          int main() {
                  action_func ptr = &TreeNode::someFunction; // assign the
   function's address to ptr TreeNode obj; (obj.*ptr)(); // call the function
                  return 0;
          }
   *
   */
 ...
 public:
   ...
   * and nonvirtual.
   *
   * If your base class destructor is virtual then objects will be destructed in
   * a order(firstly derived object then base ). If your base class destructor
   * is NOT virtual then only base class object could get deleted So there will
   ...
   * T& T::operator=(T other) noexcept // call copy or move constructor to
   * construct other
   *
   */
  TreeNode& operator=(
...
class DirectoryEntry : public ensc251::TreeNode {
 ...
 public:
  ...
  virtual void print_action() = 0;  // pure virtual function, makes this class
...
int main() {
  ...
  auto root_dir1P = std::make_shared<Directory>(
      "Dir1");  // can be std::make_unique<Directory>
  {
    // Local variable declared here, it will call its destructors at the end of
    // the block
    ...
    root_dir2 = *root_dir1P;  // copy is made for root_dir1P before assignment
                              // overload since we pass it by value
    // old root_dir2 is destroyed, therefore destructors are called

    std::cout << "root_dir2.print_traverse():" << std::endl;
    root_dir2.print_traverse();  // not in table
  }
  std::cout << "root_dir1P still valid after curly brace scope ends and "
               "root_dir2 destructor is called:"
            << std::endl;
  root_dir1P->print_traverse();
  return 0;
}
